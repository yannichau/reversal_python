...
def draw_avaiBoard(available_board, turn):
    ...
    for c in range(DIM):
        for r in range(DIM):
            pygame.draw.rect(screen, BLACK, (c*SQUARE_SIZE, (r+1)*SQUARE_SIZE, SQUARE_SIZE, SQUARE_SIZE))
            pygame.draw.rect(screen, BLUE, (c*SQUARE_SIZE, (r+1)*SQUARE_SIZE, SQUARE_SIZE, SQUARE_SIZE), 1)
            if available_board[r][c] == 1 and turn == 1: # Available moves for player 1
                pygame.draw.circle(screen, YELLOW, (int(c*SQUARE_SIZE+SQUARE_SIZE/2), int((r+1)*SQUARE_SIZE+SQUARE_SIZE/2)), RADIUS, 2)
            elif available_board[r][c] == 2 and turn == 2: # Available moves for player 2
                pygame.draw.circle(screen, RED, (int(c*SQUARE_SIZE+SQUARE_SIZE/2), int((r+1)*SQUARE_SIZE+SQUARE_SIZE/2)), RADIUS, 2)
            else:
                pass
    available_board = np.zeros((DIM,DIM))
    pygame.display.update()
